<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="closeModal">
    <div class="bg-white rounded-2xl w-full max-w-2xl shadow-2xl animate-fadeIn">
      <!-- Modal Header -->
      <div
        class="p-6 bg-gradient-to-r from-green-600 to-green-700 text-white flex justify-between items-center rounded-t-2xl">
      ...
      <div class="p-8 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
        <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-xl flex items-start gap-3">
          ...
          <p class="text-sm text-yellow-800">
            <strong>Lưu ý:</strong> Chỉ có thể sửa thông tin cơ bản. Để thay đổi lịch học, vui lòng sửa từng buổi học
            riêng lẻ.
          </p>
        </div>
        ...
        <div>
          ...
          <input v-model="formData.name" type="text" maxlength="50" placeholder="VD: Yoga Buổi Sáng"
        ...
        <div>
          ...
          <textarea v-model="formData.description" rows="4" placeholder="Mô tả về lớp học..."
            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-green-600 focus:ring-2 focus:ring-green-100 outline-none transition-all"></textarea>
        </div>
      ...
      <div class="p-6 bg-gray-50 border-t flex justify-end gap-3 rounded-b-2xl">
        ...
        <button @click="saveChanges" :disabled="!isValid || saving"
