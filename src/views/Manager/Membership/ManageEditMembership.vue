...
const planId = ref<number>(parseInt(route.params.id as string))
...
const submit = async () => {
  ...
  const payload = {
    ...
    console.log("Updating membership plan:", payload)

  try {
    const res = await api.put(`/membershipplan/${planId.value}`, payload)

    if (res.status === 200 || res.status === 201) {
            alert("Membership plan has been updated successfully!")
...
const loadPlan = async () => {
  try {
    const res = await api.get(`/membershipplan/${planId.value}`)
    const plan = res.data
    form.value = {
      name: plan.name,
    ...
    selectedTierId.value = plan.membershipTier.id
  } catch (error) {
    console.error('Failed to load membership plan:', error)
    alert("Failed to load membership plan. Please try again.")
  }
}
...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-xl font-semibold">Edit Membership Plan</h1>
      <RouterLink
        :to="{ name: 'membership' }"
