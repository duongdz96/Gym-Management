<template>
  <div class="p-6">
    ...
    <div class="flex gap-4 mb-6">
      ...
      <div class="relative flex-1">
        ...
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Tìm kiếm yêu cầu dạy..."
      ...
      >
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div>
              <span class="text-gray-500 font-semibold block flex items-center gap-1"><Users class="w-4 h-4" /> Học viên:</span>
              <span class="text-gray-800">{{ cls.memberName }}</span>
            </div>
            <div>
            ...
            <div>
              <span class="text-gray-500 font-semibold block flex items-center gap-1"><Calendar class="w-4 h-4" /> Ngày:</span>
              <span class="text-gray-800">{{ formatDate(cls.date) }}</span>
            </div>
          </div>
          ...
          <div v-if="conflictMap[cls.id]" class="p-3 bg-orange-50 border-2 border-orange-200 rounded-xl text-orange-700 font-semibold text-sm">
            <div class="flex items-center gap-2 mb-1">
              ...
              Bạn có lịch trùng với yêu cầu này
            </div>
    ...
    <div v-if="filteredClasses.length === 0" class="text-center py-16">
      ...
      <p class="text-gray-500 text-lg">Không có yêu cầu dạy cá nhân nào</p>
    </div>
  </div>
</template>
...
const availableClasses = computed(() => {
  // Show ALL personal training requests
...
const loadData = async () => {
  // Assuming there's an API for personal training requests
  classes.value = await unifiedApi.getPersonalTrainingRequests();
  roomsData.value = await unifiedApi.getRooms();
  applications.value = await unifiedApi.getPTApplications();

  // Check conflicts for all requests
  if (currentPTId.value) {
    for (const cls of classes.value) {
      try {
        const sessionData = [{
          startTime: `${cls.date}T${cls.startTime}:00`,
          endTime: `${cls.date}T${cls.endTime}:00`
        }];
        const conflicts = unifiedApi.checkPTConflicts(currentPTId.value, sessionData, cls.id);
...
const getApplicationStatus = (classId) => {
  if (!currentPTId.value) return null;
  const app = applications.value.find(a =>
    a.requestId === classId && a.ptId === currentPTId.value
  );
  return app ? app.status : null;
};

const getRejectionReason = (classId) => {
  if (!currentPTId.value) return '';
  const app = applications.value.find(a =>
    a.requestId === classId && a.ptId === currentPTId.value
  );
...
const applyToTeach = async (cls) => {
  ...
  if (conflictMap.value[cls.id]) {
    alert('❌ Bạn có lịch trùng với yêu cầu này!');
    return;
  }

  if (confirm(`Bạn có chắc muốn đăng ký dạy cho "${cls.memberName}"?`)) {
    try {
      await unifiedApi.applyToPersonalTraining(cls.id, currentPTId.value);
