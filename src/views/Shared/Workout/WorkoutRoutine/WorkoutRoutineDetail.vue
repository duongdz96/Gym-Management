...
<template>
    <div class="p-4 sm:p-6 bg-gray-50 min-h-screen">
        <button @click="goBack"
            class="mb-3 sm:mb-4 flex items-center text-sm sm:text-base text-gray-500 hover:text-gray-800 transition">
        ...
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-20 text-gray-500">
            <svg class="animate-spin h-8 w-8 text-blue-600 mb-3" xmlns="http://www.w3.org/2000/svg" fill="none"
                viewBox="0 0 24 24">
        ...
        <div v-else-if="routine">
            <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6 border border-gray-100">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 sm:mb-4 gap-3">
                    <div class="w-full md:w-auto">
                        <span
                            class="bg-emerald-100 text-emerald-600 px-2 sm:px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
                            {{ routine.muscleGroupFocus }}
                        </span>
                        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mt-2 break-words">{{ routine.name }}
                        </h1>
            ...
            <div class="space-y-3 sm:space-y-4">
                <div v-for="(detail, index) in routine.routineDetails" :key="detail.id"
                    ...
                    <div class="flex items-start gap-3 sm:gap-4 w-full md:w-auto">
                        <div
                            class="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gray-100 text-gray-500 rounded-full flex items-center justify-center font-bold text-sm">
                            {{ index + 1 }}
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1">
                                <span
                                    class="text-[10px] sm:text-xs text-gray-400 inline-block bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
            ...
            <div v-if="!routine.routineDetails || routine.routineDetails.length === 0"
                class="text-center py-10 text-gray-400 bg-white rounded-xl border border-dashed border-gray-300">
