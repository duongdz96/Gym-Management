...
def collect_face_embeddings(person_id, person_name, person_type='member'):
    ...
    print("  \u2713 Keep face still during capture")
    print("  \u2713 Remove glasses if possible (or collect both)")
    ...
    while len(embeddings) < config.NUM_SAMPLES:
        ...
        # Instructions
        cv2.putText(display_frame, "Place face in center", (50, 100),
        ...
        try:
            ...
            if len(face_objs) > 0:
                ...
                if len(face_objs) > 1:
                    cv2.putText(display_frame, "Multiple faces detected! Show only 1 person", (50, 150),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
            else:
                # No face detected
                cv2.putText(display_frame, "No face detected - Place face in view", (50, 150),
